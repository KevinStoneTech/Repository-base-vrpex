...
Citizen.CreateThread(function()
    ...
    if GetEntityHealth(PlayerPedId()) <= 160 then
    	setHurt()
	elseif hurt and GetEntityHealth(PlayerPedId()) > 180 then
    	setNotHurt()
	end
	
	if GetEntityHealth(PlayerPedId()) <= 100 and not mostrando then
		mostrando = true
		vRP._setDiv("parte5",css,"<img src=\"https://cdn.discordapp.com/attachments/754256628831748097/760655881548726272/BLUUU.png\" class=\"porra\"><h1><br>Você está desacordado</h1><br><br><br><h2>Por favor, siga estas regras!</h2><br><br><h3>Em caso de reanimação, por um médico, simule um estado de debilitação.<br><br>Apenas se Comunique com o Medico<br><br>Caso não seja reanimado, você perderá sua memória. Portanto, você está proibído de utilizar os eventos anteriores a seu favor ou não.</h3><h3 style=\"opacity: 0.5;font-size: 20px;\">P.S: Mesmo que seja reanimado por um médico você não poderá se lembrar do ocorrido.(RL).</h3>")
	elseif GetEntityHealth(PlayerPedId()) >= 101 then
		mostrando = false
		vRP._removeDiv("parte5")
	end
