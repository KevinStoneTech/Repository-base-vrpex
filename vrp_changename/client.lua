...
function checkDistance()
	...
	local distance = GetDistanceBetweenCoords(x,y,cdz,-550.84,-191.97,38.23,true)
	if distance < 5 then
		return true
	else
		return false
...
Citizen.CreateThread(function()
	while true do
		...
		if checkDistance() then
			sleep = 4
			DrawMarker(23,-550.84,-191.97,38.23-0.99,1,1,0,0,0,0,1.0,1.0,1.0,255,255,255,50,0,1,0,0)
			DrawText3D(-550.84,-191.97,38.23, "Mudar Nome", 2.0, 7, 100)
			DrawText3D(-550.84,-191.97,38.23-0.4, "/identidade [nome] [sobrenome] [idade]", 2.0, 1, 100)
		end	
		Citizen.Wait(sleep) 
	end
end)
