...
RegisterCommand('countdown',function(source,args,rawCommand)
    ...
    if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"mod.permissao") then
        ...
        if parseInt(args[1]) > 0 then
            ...
            TriggerClientEvent('chatMessage',-1,"[SISTEMA]",{255,160,0},"Servidor reiniciando em "..countdown_time.." minutos.")
...
RegisterServerEvent("minharolafasfsa")
AddEventHandler("minharolafasfsa",function()
	local user_id = vRP.getUserId(source)
	if user_id then
